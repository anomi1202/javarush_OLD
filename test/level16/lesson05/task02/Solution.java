...
2. Посчитать количество финишировавших лошадей и возвратить его. Используй метод isFinished().
2. Если лошадь еще не пришла к финишу (!isFinished()), то:
2.2. Вывести в консоль "Waiting for " + horse.getName().
...
public class Solution {
    ...
    public static int calculateHorsesFinished(List<Horse> horses) throws InterruptedException {
        ...
        for (Horse horseNubmer : horses) {
            if (horseNubmer.isFinished()){
                countFinished += 1;
            }
            else {
                System.out.println("Waiting for " + horseNubmer.getName());
                horseNubmer.join();
            }
        }
